...
def sample_model(
    ...
    if batched:
        ...
        for i in range(n_sample_per_prompt):
            genseqs, genscores = run_model(
                prompt_batches[i], model, tokenizer, n_tokens,
                temp, top_k, batched, device, force_prompt_threshold
            )
            current_batch = [
                [uuid.uuid4().hex, prompt, seq, str(score)]
...
def filter_protein_fasta(
    ...
    def check_segmasker_installation() -> None:
       ...
       try:
           result = subprocess.run([segmasker_path, '-version'], 
    ...
    def passes_length_and_partial_filters(record: str, min_length: int = 40, max_length: int = 1200, filter_partial_bool: bool = False) -> bool:
    ...
    filtered_records = [
        ...
        if passes_quality_filters(str(record.seq), segmasker_threshold) and passes_length_and_partial_filters(record,min_length,max_length,filter_partial_bool)
    ]
...
def run_hmmsearch(input_fasta: str, hmm_folder: str, output_csv: str, n_threads: int = 1, incE: float = 0.2) -> pd.DataFrame:
    """
    ...
    for hmm_file in os.listdir(hmm_folder):
        if hmm_file.endswith('.hmm3'):
            ...
            try:
                ...
                subprocess.run([
                    'hmmsearch',
                    '--domtblout', temp_output_path, # Output results to a table
                    '--cpu', str(n_threads),         # Number of parallel threads
                    '--incE', str(incE),              # E-value threshold for inclusion
...
def get_pfam_hits(input_fasta: str, pfam_db_path: str, output_csv: str, n_threads: int = 1, verbose: bool = False) -> pd.DataFrame:
    """
    ...
    with tempfile.NamedTemporaryFile(delete=False, mode='w', suffix='.txt') as tmp_hmmscan_output:
        ...
        columns = [
            ...
            "envfrom", "env to", "acc", "description of target"
        ]
        pfam = []
...
def fold_proteins(input_file: str, output_file: str, device: str = 'cuda:0') -> pd.DataFrame:
    ...
    for record in SeqIO.parse(input_file, "fasta"):
        ...
        with torch.inference_mode():
            ...
            avg_plddt = (esmfold_out["plddt"] * esmfold_out["atom37_atom_exists"]).sum(dim=(1, 2)) / esmfold_out["atom37_atom_exists"].sum(dim=(1, 2)).item()
    ...
    aa_fold_stats = pd.DataFrame(folds, columns=['Amino Acid Sequence', 'Evo Sequence ID', 'PDB Output', 'Average pLDDT', 'pTM'])
    aa_fold_stats.to_csv(output_file, index = False)
    return aa_fold_stats

def filter_proteins_by_threshold(df: pd.DataFrame, output_file: str, plddt_threshold: float = 0.4, ptm_threshold: float = 0.4) -> pd.DataFrame:
    """
    ...
    with tempfile.TemporaryDirectory() as tmp_path:
        ...
        foldseek_df = pd.DataFrame(
            ...
            columns=['Amino Acid Sequence', 'Sequence ID', 'Query', 'Target',
                    'Alignment TM-score', 'LDDT', 'Probability']
        )
