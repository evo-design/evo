...
class Config:
    ...
    output_dir: Path  # Directory to store generated artifacts
    ...
    shared_protein_pairs_csv: Path = field(init=False)
    cofold_fasta: Path = field(init=False)
    ...
    def __post_init__(self) -> None:
        """Resolve configured paths and enumerate all downstream artifact filenames."""
        ...
        self.shared_protein_pairs_csv = self.output_dir / "shared_protein_pairs.csv"
        self.cofold_fasta = self.output_dir / "cofold_targets.fasta"
...
def csv_to_cofold_fasta(
    ...
    Returns:
        None. Writes FASTA entries in the format:
...
def load_config(config_file: str) -> Config:
    """Load configuration from a YAML file path and instantiate `Config`.

    Args:
        config_file: Path to the YAML config file.

    Returns:
        Config instance with derived paths created.
    """
    try:
...
def process_sequences(
    ...
    """Generate toxin/antitoxin sequences and run baseline QC.

    Args:
        config: Runtime configuration with IO paths and thresholds.
        model: Language model used for generation.
        tokenizer: Tokenizer associated with the model.

    Returns:
        None. Writes intermediate artifacts to disk.
    """
...
def process_folds(config: Config) -> pd.DataFrame:
    """Fold filtered proteins and return those exceeding the confidence cutoffs.

    Args:
        config: Runtime configuration containing ESMFold thresholds.

    Returns:
        DataFrame of proteins that passed pLDDT/pTM filters.
    """
...
def main(config_file: str) -> None:
    """Execute the toxin-antitoxin sampling workflow from a YAML config path.
    ...
    try:
        ...
        if config.run_esm_fold:
            ...
            protein_pairs = identify_unique_pairs(
                filtered_folds, config.shared_protein_pairs_csv
            )
