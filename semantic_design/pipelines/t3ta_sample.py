...
class Config:
    ...
    # Core file locations (prompts, binaries, and reference tables)
    input_prompts: Path
    output_dir: Path
    segmasker_path: Path
    trf_path: Path
    rna_structures_reference_csv: Path
    ...
    # RNA/TRF post-processing (structure/sequence heuristics)
    write_trf_to_csv: bool = True
    rna_structure_filter_reference_csv: Optional[Path] = None
    rna_structure_filter_script: Optional[Path] = None
    ...
    rna_sequence_filter_script: Optional[Path] = None
    ...
    hmmscan_cpu: int = 4
    pfam_allowed_names: Optional[List[str]] = None
    ...
    def __post_init__(self) -> None:
        """Normalize user-supplied paths and construct all derived artifact locations."""
        ...
        if self.rna_structure_filter_script:
            self.rna_structure_filter_script = Path(self.rna_structure_filter_script)
...
def load_config(config_path: Path) -> Config:
    """Load a YAML config file and instantiate a `Config`.

    Args:
        config_path: Path to the YAML file describing the run.

    Returns:
        Fully initialized `Config` object with derived paths created.
    """
...
def load_generated_sequences(csv_path: Path) -> Dict[str, str]:
    """Return a mapping of generation UUID/root IDs to their DNA sequences.

    Args:
        csv_path: Path to the CSV produced by Evo sampling.

    Returns:
        Dictionary keyed by sequence identifier with DNA strings as values.
    """
...
def run_tandem_repeat_finder(
    ...
    Args:
        ...
        root_id: Identifier of the originating sample.
        trf_path: Path to the TRF binary.

    Returns:
        DataFrame describing each detected tandem repeat region.
    """
...
def get_tandem_repeats(
    ...
    """Run TRF across every filtered sequence and optionally persist the table.

    Args:
        filtered_folds: Table of proteins that survived structural QC.
        sequences_csv: Path to the CSV of generated DNA sequences.
        config: Global configuration with TRF locations/output paths.

    Returns:
        DataFrame concatenating all TRF hits for the surviving roots.
    """
...
def predict_rna_structure(rna_sequence: str) -> Tuple[str, float]:
    """Fold an RNA sequence using ViennaRNA and return the structure + MFE."""
...
def predict_hairpins(dot_bracket: str) -> List[Tuple[int, int, int, int]]:
    """Identify simplistic hairpins inside dot-bracket structures.

    Args:
        dot_bracket: Secondary structure string generated by ViennaRNA.

    Returns:
        List of (stem_start, stem_end, loop_start, loop_end) tuples.
    """
...
def fold_trfs(trf_df: pd.DataFrame, output_csv: Path) -> pd.DataFrame:
    """Convert TRF windows to RNA, fold with ViennaRNA, and persist the table.

    Args:
        trf_df: Tandem repeat table returned by TRF.
        output_csv: Destination CSV for the fold results.

    Returns:
        DataFrame describing the folded TRF windows with hairpin metadata.
    """
    if trf_df.empty:
        empty = pd.DataFrame(
            columns=[
...
def get_at_pairs(
    ...
    Args:
        rna_fold_df: Folded TRF table containing sequences + hairpins.
        filtered_folds: Protein table after all structural filters.
        output_csv: Path where the merged table should be written.

    Returns:
        DataFrame joining hairpin-positive RNAs to their cognate proteins.
    """
...
def visualize_rna_structures(
    ...
    Args:
        ...
        rna_fold_df: Folded TRF dataframe for generated candidates.

    Returns:
        Tuple of (feature_vectors, labels) for downstream visualization.
    """
...
def _hairpin_exists(hairpin_value: Any) -> bool:
    """Return True if a parsed or stringified hairpin field contains data."""
...
def _has_all_bases(dna_sequence: Any) -> bool:
    """Return True if the DNA sequence contains at least one of A/C/G/T."""
...
def filter_folded_trfs(
    ...
    """Apply simple QC (hairpins, MFE, base composition) to folded TRFs.

    Args:
        trf_df: Raw TRF table from tandem repeat finder.
        fold_trf_df: RNA fold dataframe.
        config: Global configuration containing QC thresholds/flags.

    Returns:
        Filtered fold table plus the set of passing root IDs.
    """
...
def write_rna_candidates_fasta(candidate_table: pd.DataFrame, fasta_path: Path) -> None:
    """Emit TRF-derived RNA sequences to FASTA for downstream cmscan usage.

    Args:
        candidate_table: DataFrame returned by prepare_rna_candidate_table.
        fasta_path: Destination FASTA path to create/overwrite.
    """
...
def prepare_rna_candidate_table(
    ...
    Args:
        trf_df: Tandem repeat dataframe.
        fold_trf_df: Folded TRF dataframe with hairpin info.
        output_csv: Destination CSV file.

    Returns:
        DataFrame containing sequence metadata plus consensus/full fold fields.
    """
...
def run_rna_structure_filter(candidates_csv: Path, config: Config) -> Set[str]:
    ...
    Args:
        ...
        config: Configuration block with script path/reference CSV.

    Returns:
        Set of `Root ID` values whose RNAs passed the structural search.
    """
...
def run_rna_sequence_filter(candidates_csv: Path, config: Config) -> Set[str]:
    ...
    Returns:
        Set of root IDs whose RNAs exceed the minimum identity threshold.
    """
...
def run_hmmscan_filter(config: Config) -> pd.DataFrame:
    """Run hmmscan on filtered proteins and return hits that match whitelist + E-value.

    Args:
        config: Configuration with binary path, Pfam database, and hit CSV paths.
...
def load_allowed_pfam_names(names: Optional[List[str]]) -> Set[str]:
    """Normalize Pfam whitelist entries when explicitly provided."""
    if not names:
        return set()
    return {str(name).strip().strip('"') for name in names if str(name).strip()}


def parse_cmscan_tblout(tblout_path: Path) -> pd.DataFrame:
    """Parse Infernal cmscan --tblout output into a DataFrame."""
...
def run_cmscan_filter(candidate_table: pd.DataFrame, config: Config) -> pd.DataFrame:
    """Run cmscan against provided CMs and filter hits by target family + E-value.

    Args:
        candidate_table: RNA candidate dataframe used for ID lookups.
        config: Configuration detailing cmscan binaries/models/filters.

    Returns:
        DataFrame of cmscan hits that met the whitelist + E-value criteria.
    """
...
def run_pipeline(config_path: Path) -> None:
    """High-level orchestration for Type III TA sampling, filtering, and reporting.

    Args:
        config_path: Path to YAML configuration describing the run.

    Returns:
        None. Writes all side-effect artifacts into `Config.output_dir`.
    """
