...
def extract_structure_features(structure: str, mfe: float, hairpins: str) -> np.ndarray:
    ...
    return np.array([
        length, num_pairs, pairing_ratio, avg_stem_length, avg_loop_length,
        max_stem_length, num_stems, mfe_val, mfe_per_nucleotide, num_hairpins
    ])


def create_structure_index(df: pd.DataFrame, structure_type: str = "both") -> Dict[str, np.ndarray]:
...
def cosine_similarity_filter(query_features: np.ndarray, target_features: np.ndarray, threshold: float = 0.8) -> np.ndarray:
...
def create_structural_motif_index(df: pd.DataFrame, structure_type: str = "both") -> Dict[str, Dict[str, object]]:
...
def motif_similarity_filter(query_motifs: str, target_motif_matrix, vectorizer, threshold: float = 0.3) -> np.ndarray:
...
def pure_structural_pre_filter(query_structure: str, target_structures: List[str], threshold: float = 0.4) -> np.ndarray:
...
def optimized_similarity_search(
    ...
    for struct_type in structure_types:
        ...
        for _, target_row in target_df.iterrows():
            ...
            target_structures.append(target_struct if not pd.isna(target_struct) else "")
        ...
        print(f"  {struct_type.title()} Stage 1: {len(quick_candidates)}/{len(target_structures)} candidates after structural pre-filter")
        ...
        if len(quick_candidates) > 0:
            ...
            motif_candidates_rel = motif_similarity_filter(
                query_motifs, candidate_motif_matrix, motif_index['vectorizer'], threshold=0.2
            )
            motif_candidates = quick_candidates[motif_candidates_rel] if len(motif_candidates_rel) > 0 else np.array([])
        else:
        ...
        print(f"  {struct_type.title()} Stage 2: {len(motif_candidates)} candidates after motif filter")
        ...
        if len(motif_candidates) > 0:
            ...
            final_candidates = motif_candidates[feature_candidates_rel] if len(feature_candidates_rel) > 0 else np.array([])
        else:
        ...
        print(f"  {struct_type.title()} Stage 3: {len(final_candidates)} candidates after feature filter")
        ...
        for target_idx in final_candidates:
            ...
            if struct_sim >= min_similarity:
                ...
                if combined_score >= min_similarity:
                    ...
                    complementary_type = 'full' if struct_type == 'consensus' else 'consensus'
                    comp_query_structure = query_row.get(f'{complementary_type.title()} Structure', '')
                    comp_target_structure = target_row.get(f'{complementary_type.title()} Structure', '')
                    ...
                    if comp_query_structure and comp_target_structure and not pd.isna(comp_query_structure) and not pd.isna(comp_target_structure):
                        comp_struct_sim = 1.0 - base_pair_distance(comp_query_structure, comp_target_structure)
                        
                        comp_query_mfe = query_row.get(f'{complementary_type.title()} MFE', None)
                        comp_target_mfe = target_row.get(f'{complementary_type.title()} MFE', None)
                        
                        if not pd.isna(comp_query_mfe) and not pd.isna(comp_target_mfe):
                            comp_mfe_diff = abs(float(comp_query_mfe) - float(comp_target_mfe))
                    ...
                    result = {
                        ...
                        'Target_Hairpins': target_row[f'{struct_type.title()} Hairpins'],
                        ...
                        'Query_Complementary_MFE': query_row.get(f'{complementary_type.title()} MFE', None),
                        'Target_Complementary_MFE': target_row.get(f'{complementary_type.title()} MFE', None),
                        'Query_Complementary_Hairpins': query_row.get(f'{complementary_type.title()} Hairpins', []),
                        'Target_Complementary_Hairpins': target_row.get(f'{complementary_type.title()} Hairpins', []),
...
def optimized_structural_search(
    ...
    for batch_num in range(total_batches):
        ...
        print(f"Processing batch {batch_num + 1}/{total_batches} (sequences {batch_start}-{batch_end})")
        ...
        print(f"Batch {batch_num + 1} completed. Running total: {len(all_results)} matches")
...
def main() -> Optional[pd.DataFrame]:
    parser = argparse.ArgumentParser(description='Optimized RNA Structure Similarity Search')
    ...
    parser.add_argument('--structure-type', choices=['consensus', 'full', 'both'], 
                       default='both', help='Structure types to compare')
    parser.add_argument('--min-similarity', type=float, default=0.7,
                       help='Minimum similarity threshold')
    parser.add_argument('--pre-filter-threshold', type=float, default=0.6,
